...
let rec eval_expr (e : expr) : value comp =
  ...
  | Binop (op, e1, e2) ->
     ...
     eval_op op v1 v2
  | Var x -> deref x
  | PostInc x -> fun h -> let v = fst (deref x h) in let nv = fst (int_op (+) v (VInt 1) h)  in (nv , snd (assgn x nv h))
  | PostDec x -> fun h -> let v = fst (deref x h) in let nv = fst (int_op (-) v (VInt 1) h)  in (nv , snd (assgn x nv h))
  | PreInc x -> fun h -> let v = fst (deref x h) in let nv = fst (int_op (+) v (VInt 1) h)  in (v , snd (assgn x nv h))
  | PreDec x -> fun h -> let v = fst (deref x h) in let nv = fst (int_op (-) v (VInt 1) h)  in (v , snd (assgn x nv h))
