...
let stack_size (p : prog) : int = 
	let rec eval (s : I.value list) (p : prog) (info : int*int) : int =
	  match p, s, info with
	  | [], [_], (a,b) -> (max a b)
	  | [], _  , (_,_) -> failwith "error!"
	  | (PushInt n :: p'), _ , (a,b)->
	      eval (I.VInt n :: s) p' (a+1, max (a+1) b)
	  | (PushBool bo :: p'), _ , (a,b)->
	      eval (I.VBool bo :: s) p' (a+1, max (a+1) b)
	  | (Binop op :: p'), (v2 :: v1 :: s'), (a,b) ->
	      eval (I.eval_op op v1 v2 :: s') p' (a-2, max a b)
	  | (CndJmp (t,_) :: p'), (I.VBool true :: s'), (a,b) ->
	      eval s' (t @ p') (a-1, max a b)
	  | (CndJmp (_,e) :: p'), (I.VBool false :: s') , (a,b)->
	      eval s' (e @ p') (a-1, max a b)
	  | (PushPair :: p'), (v2 :: v1 :: s') , (a,b)->
	      eval (I.VPair (v1, v2) :: s') p' (a-1, max a b)
	  | (Fst :: p'), (I.VPair (v1,_) :: s') , (a,b)->
	      eval (v1 :: s') p' (a, max a b)
	  | (Snd :: p'), (I.VPair (_,v2) :: s') , (a,b)->
	      eval (v2 :: s') p' (a, max a b)
	  | (PushUnit :: p'), (s') , (a,b)->
	      eval (I.VUnit :: s') p' (a+1, max (a+1) b)
