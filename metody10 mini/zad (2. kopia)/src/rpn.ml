...
let rec ast_to_pn (expr : Ast.expr) : prog =
  match expr with
  | Ast.Int n ->
      [PushInt n]
  | Ast.Bool b ->
      [PushBool b]
  | Ast.Binop (op, e1, e2) ->
       [Binop op] @ ast_to_pn e1 @ ast_to_pn e2
  | Ast.If (b, t, e) ->
      [CndJmp (ast_to_pn t, ast_to_pn e)] @ ast_to_pn b
  | Ast.Pair (e1, e2) ->
      [PushPair] @  ast_to_pn e1 @ ast_to_pn e2
  | Ast.Fst e ->
      [Fst] @ ast_to_pn e 
  | Ast.Snd e ->
       [Snd] @ ast_to_pn e
  ...
  | Ast.IsPair e ->
       [IsPair] @ ast_to_pn e
  | _ ->
      failwith "sCamlenie wielblada!"


type cmd_or_value = | Value of I.value | Cmd of cmd

let rec eval_pn (p : prog) (s : cmd_or_value list)  : I.value =
  match p, s with
	| [],[Value n] -> n
	| _ , ((Cmd ( PushInt n )) :: s') -> eval_pn p ( (Value ( I.VInt n ) )::s' )
	| _ , ((Cmd ( PushBool n )) :: s') -> eval_pn p ( (Value ( I.VBool n ) )::s' )
        | _ ,  (Value v2) :: (Value v1) :: (Cmd(Binop op)) :: s' -> eval_pn p ( (Value (I.eval_op op v1 v2) )  ::s' )
	| _ , 	(Value (I.VBool true))  :: (Cmd (CndJmp (t,_))) :: s'  -> eval_pn (t @ p) s' 	
        | _ , 	(Value (I.VBool false)  :: Cmd (CndJmp (_,e)) :: s')  -> eval_pn (e @ p) s' 	
	| _, (  (Value v2) :: (Value v1) :: Cmd(PushPair) :: s') -> eval_pn p (Value (I.VPair (v1, v2)) :: s') 
	| _, (Cmd Fst) :: (Value (I.VPair (v1,_))) :: s'  ->    eval_pn p (Value v1 :: s')
	| _, (Cmd Snd) :: (Value (I.VPair (_,v2))) :: s'  ->   eval_pn p (Value v2 :: s')
	| _, (Cmd(PushUnit) :: s') ->      eval_pn p (Value(I.VUnit) :: s')
	| command :: p' ,_ -> eval_pn p' ((Cmd command) :: s)
	| _ -> failwith "sCamlenie wielblada!"

(*
utop # eval_pn (ast_to_pn (parse "(if 2 + 2 > 3 then 2137 else 666, (333 - 233,()))")) [];;
- : value = VPair (VInt 2137, VPair (VInt 100, VUnit))
*)
